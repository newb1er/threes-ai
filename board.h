...
class board {
 ...
 public:
  ...
  static unsigned ttov(unsigned t) {
    return t >= 3 ? std::pow(3, std::log2(t / 3) + 1) : 0;
  }
 ...
 private:
  grid tile;
  data attr;  // (#3-tile:4-bit) (#2-tile:4-bit) (#1-tile:4-bit)
              // (last_action:4-bit) (hint_tile:4-bit)
};
